...
Chỉ tiến hoá từ giả thuyết đã được đánh giá đầy đủ (có full review và đã đấu ít
nhất 1 trận). Giả thuyết mới sinh ra không ghi đè bản gốc — nó phải tự qua
review và tournament như mọi giả thuyết khác.
"""
...
class EvolutionAgent(BaseAgent):
    ...
    async def run(self, top_k: int = 4) -> List[Hypothesis]:
        # Chỉ tiến hoá giả thuyết đã review + đã đấu: Elo của chúng có ý nghĩa.
        top = self.memory.get_top_k(top_k, evaluated_only=True)
        if not top:
            return []
        ...
        for r in results:
            if isinstance(r, Exception):
                logger.warning("Evolution: 1 biến thể lỗi, bỏ qua: %s", r)
                continue
            self.memory.add_hypothesis(r)
            evolved.append(r)
        return evolved
